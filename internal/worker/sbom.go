...
func (r *Runner) generateSbomExports(ctx context.Context, jobID, reportPath, reportBucket, reportKey string) int {
	...
	for _, f := range formats {
		...
		// Keep CycloneDX on disk — generateSbomDiff needs it; scratch cleanup handles removal
		if f.name != "cyclonedx" {
			os.Remove(outPath)
		}
		success++
...
func (r *Runner) generateSbomDiff(ctx context.Context, jobID, orgID, objectKey, sourceType string, registryImage *string, currentReportPath, reportBucket, reportKey string) {
	...
	// Download the baseline's CycloneDX SBOM from S3 (not the raw report)
	...
	baselineSbomKey := strings.TrimSuffix(*prev.ReportKey, ".json") + ".sbom.cdx.json"
	baselinePath := filepath.Join(baselineDir, "baseline.sbom.cdx.json")
	if err := r.s3.DownloadToFile(ctx, *prev.ReportBucket, baselineSbomKey, baselinePath, nil); err != nil {
		log.Printf("[job=%s] sbom diff: failed to download baseline SBOM (%s/%s): %v", jobID, *prev.ReportBucket, baselineSbomKey, err)
		return
	}

	// Use the current scan's CycloneDX SBOM export (generated by generateSbomExports before this call)
	currentSbomPath := currentReportPath + ".sbom.cdx.json"
	if _, err := os.Stat(currentSbomPath); err != nil {
		log.Printf("[job=%s] sbom diff: current SBOM not found at %s: %v", jobID, currentSbomPath, err)
		return
	}
